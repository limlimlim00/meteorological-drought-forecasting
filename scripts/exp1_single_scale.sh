#!/usr/bin/env bash
# Experiment 3-A: Single-scale input (단일 스케일 입력)
# 입력 = 예측 대상과 동일한 SPI 스케일 (선행연구 방식)
#
# Models : CNN, LSTM(S-LSTM), BiLSTM(S-BiLSTM), CNN-LSTM
# Targets: SPI1, SPI3, SPI6, SPI9, SPI12, SPI18, SPI24
# Usage  : bash scripts/exp3_single_scale.sh [--dry-run]
...
run_or_skip() {
    ...
    if [ -d "results/experiments/$rid" ]; then
        ...
        (( RUN++ ))
        echo "▶  $rid"
        $DRY_RUN || python "$@" 2>&1 | tee -a "$LOG_DIR/exp3_single_$(date +%Y%m%d).log"
    fi
}
...
for lr          in 1e-3 5e-4; do
    rid="cnn_${station}_${target}_sl${seq_len}_hs${hidden_size}_nl2_ks${kernel_size}_dr${dropout}_lr${lr}_fSPI${target#SPI}"
    # features = target 자신만
...
for dropout     in 0.0 0.1; do
for lr          in 1e-3 5e-4; do
    rid="lstm_${station}_${target}_sl${seq_len}_hs${hidden_size}_nl${num_layers}_dr${dropout}_lr${lr}_bs32_fSPI${target#SPI}"
    run_or_skip "$rid" $SCRIPT \
        --model lstm \
        --station "$station" --target "$target" \
        --features "$target" \
...
for dropout     in 0.0 0.1; do
for lr          in 1e-3 5e-4; do
    rid="bilstm_${station}_${target}_sl${seq_len}_hs${hidden_size}_nl${num_layers}_dr${dropout}_lr${lr}_bs32_fSPI${target#SPI}"
    run_or_skip "$rid" $SCRIPT \
        --model bilstm \
        --station "$station" --target "$target" \
        --features "$target" \
...
for lr           in 1e-3 5e-4; do
    ch="${cnn_channels//,/-}"
    rid="cnn_lstm_${station}_${target}_sl${seq_len}_ch${ch}_lh${lstm_hidden}_ll${lstm_layers}_ks3_dr${dropout}_lr${lr}_fSPI${target#SPI}"
    run_or_skip "$rid" $SCRIPT \
        --model cnn_lstm \
        --station "$station" --target "$target" \
        --features "$target" \
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  [Exp3-A: Single-scale] skip: $SKIP  |  run: $RUN"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
